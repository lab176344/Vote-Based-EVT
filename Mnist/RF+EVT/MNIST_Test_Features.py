...
for x in range(0, 5):
    ...
    x_train_sixes = x_train[y_train == label[6]]
    layer_ouput_sixes = get_1st_layer_output([x_train_sixes])[0]
    layer_output = np.concatenate((layer_output, layer_ouput_sixes[m_checkFeatures:m_Itotalfeatures]), axis=0)
    x_trainout=np.concatenate((x_trainout,x_train_sixes[m_checkFeatures:m_Itotalfeatures]),axis=0)
    print('7 shape:', layer_output.shape)

    x_train_seven = x_train[y_train == label[7]]
    layer_ouput_seven = get_1st_layer_output([x_train_seven])[0]
    layer_output = np.concatenate((layer_output, layer_ouput_seven[m_checkFeatures:m_Itotalfeatures]), axis=0)
    x_trainout=np.concatenate((x_trainout,x_train_seven[m_checkFeatures:m_Itotalfeatures]),axis=0)
    print('8 shape:', layer_output.shape)

    x_train_eight = x_train[y_train == label[8]]
    layer_ouput_eight = get_1st_layer_output([x_train_eight])[0]
    layer_output = np.concatenate((layer_output, layer_ouput_eight[m_checkFeatures:m_Itotalfeatures]), axis=0)
    x_trainout=np.concatenate((x_trainout,x_train_eight[m_checkFeatures:m_Itotalfeatures]),axis=0)
    print('9 shape:', layer_output.shape)

    x_train_nine = x_train[y_train == label[9]]
    layer_ouput_nine = get_1st_layer_output([x_train_nine])[0]
    layer_output = np.concatenate((layer_output, layer_ouput_nine[m_checkFeatures:m_Itotalfeatures]), axis=0)
    x_trainout=np.concatenate((x_trainout,x_train_nine[m_checkFeatures:m_Itotalfeatures]),axis=0)
    ...
    y_train_sorted = np.concatenate((y_train_sorted, 6 * np.ones((m_Itotalfeatures - m_checkFeatures,), dtype=int)), axis=0)
    y_train_sevens = y_train[y_train == label[7]]
    y_train_sorted = np.concatenate((y_train_sorted, 7 * np.ones((m_Itotalfeatures - m_checkFeatures,), dtype=int)), axis=0)
    y_train_eights = y_train[y_train == label[8]]
    y_train_sorted = np.concatenate( (y_train_sorted, 8 * np.ones((m_Itotalfeatures - m_checkFeatures,), dtype=int)), axis=0)
    y_train_nines = y_train[y_train == label[9]]
    y_train_sorted = np.concatenate((y_train_sorted, 9 * np.ones((m_Itotalfeatures - m_checkFeatures,), dtype=int)), axis=0)
    output = {}
    saveName = 'Test\TestData' + str(x)+'.mat'

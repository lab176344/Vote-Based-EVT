...
for q = 1:5
    ...
    load(mat(q).name);
    if(Calib)
        TrainDataSet=CalibDataSet;
        TrainTarget=CalibTarget;
    else
        TrainDataSet=TestDataSet;
        TrainTarget=TestTarget;
        
    end
    TrainDataSet=reshape(TrainDataSet,size(TrainDataSet,1),512);
    ...
    saveName=strcat('EVTCalib\',mat(q).name,'Calib.mat');
    save(saveName,'confidence','outputRF');
end

...
def main():
    for ranCheck in range(5):
        ...
        # Load the mat file
        dataDir = 'HighDScenario9Class.mat'
        ...
        label_gen=random.sample(range(9),9)
        ...
        utrain_s, utrain_l,utest_s,utest_l = get_few_class(urand_label, 4, X_train, y_train, X_test, y_test)
        train_test = True
        if train_test:
            X_train   = train_s
            y_train   = train_l
        ...
        for classCheck in range(4):
            ltrain_s,ltrain_l = map(np.array, zip(*[(x,5) for (x,y) in zip(x_openset, y_openset)
        ...
        save_train_features = 'RF+EVT\Train\Train_Features'+str(ranCheck)+'.mat' 
        sio.savemat(save_train_features, output_Train)

        save_test_features = 'RF+EVT\Train\Calib\Calib_Features'+str(ranCheck)+'.mat' 
        ...
        save_open_features = 'RF+EVT\Train\Test\Test_Features'+str(ranCheck)+'.mat' 
